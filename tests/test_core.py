"""Tests for pinescan.core — the reusable Pine->Python conversion pipeline.
...
Run: python -m pytest tests/test_core.py   (also runnable directly)
"""
import io
import json
import math
...
from pinescan.core import lint as pl
from pinescan.core import parity as pp
...
def test_cli_lint_json():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        with open(p, "w", encoding="utf-8") as f:
            f.write(SAMPLE_PINE)
        r = subprocess.run(
            [sys.executable, "-m", "pinescan.core", "lint", p, "--json"],
