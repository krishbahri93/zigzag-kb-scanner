...
def _nested_peaks(points, b_price, b_idx, min_gap_pct, max_swings):
    ...
    for j in range(n):
        ...
        if t < b_idx:
            pr = points[idx][1]
            # Pine builds the peak list from zzP.pivots (segments) and derives each
            # point's flag from the SEGMENT direction (Pine 146-156: down = end<start,
