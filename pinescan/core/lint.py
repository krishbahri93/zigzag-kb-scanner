...
Run this FIRST, before writing any Python. It answers, deterministically:
  ...
  * Which ta./math. builtins are used, and which have no Pine-exact
    implementation in pinescan.core.runtime yet?
...
humans. CLI: python -m pinescan.core lint <file.pine> [--json]
"""
import re
...
def lint_pine(text):
    ...
    if re.search(r"\bvar\s+\w", src):
        warnings.append(_warn(
            ...
            "dependent. Port bar-by-bar (pinescan.core.runtime.Series); never "
            "vectorize.",
        ))
    ...
    if unsupported:
        warnings.append(_warn(
            ...
            "No Pine-exact implementation in pinescan.core.runtime yet for: "
            + ", ".join(unsupported)
...
def format_report(report, path="<source>"):
    ...
    lines = [f"pinescan.core lint — {path}", "=" * 60]
