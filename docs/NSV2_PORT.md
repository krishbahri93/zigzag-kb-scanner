...
built with the `pinescan.core` pipeline. Engine: `nsv2_engine.py`. Tests:
...
3. Run the diff:
   ```
   python -m pinescan.core parity --csv your_export.csv --port pinescan.nsv2_engine:run --snapshot nsv2_golden.json
   ```
