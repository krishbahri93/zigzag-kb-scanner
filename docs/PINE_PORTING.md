...
> **Tooling:** this checklist is now executable. The `pinescan/core/` package
> automates the deterministic steps — `python -m pinescan.core lint <file.pine>`
> (step 1 + trap flags + plot inventory) and `python -m pinescan.core parity
> --csv golden.csv --port module:run` (step 7, incl. regression snapshots) —
> and `pinescan/core/runtime.py` provides Pine-exact `ta.*` building blocks for
> step 6. The agent workflow lives in `.claude/skills/pine-to-python/`.
> See `pinescan/core/README.md`.
